...
- 2026-09-26: 📋 copy as text — dumps all five notes as HEADER + text blocks (empty note → —) via clipboard API with an execCommand fallback; flushes the pending autosave first so "saved" can't overwrite the "copied" message. Real og.png (staged board with sample notes) replaces the remote pollinations image; og:url → app.sloppy.live.
...
- (done 2026-09-26: copy as text) — a print stylesheet could still be nice.
