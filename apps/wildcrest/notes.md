...
## log
- 2026-04-19: Expansion — evolution system (5 base → 5 evolved forms at Lv10, in-place swap preserving HP ratio, 1.6s evolve animation), elemental chart overlay (press C, 5×5 type grid colored green/red/neutral), shop (Moss & Mint Emporium: Potion/Super Potion/Revive Leaf/Rage Tonic, coins from wild wins), inventory + Item battle menu (3-option main: Fight/Item/Flee), gym leader Warden Thorne (3-creature party: Mosshoot L10 → Tidalhound L11 → Granikoid L12, can't flee, 80c + Crest Badge on win). Added shop/building/leader tiles (cobblestone shop with awning, brick walls, farmer sprite with pulsing alert flag). HUD shows coins + badge. Shop moved onto path loop for reachability.
