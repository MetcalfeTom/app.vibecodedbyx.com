...
## log
- v1.2 (2026-09-14, 0d884884f): real-twin photos per chat — local upload (canvas downscale ≤512px/150KB, 2.5MB shelf cap, localStorage only) + URL previews gated to plain http(s) (javascript:/data:/ftp: refused pre-load) and VALIDATED by an actual load w/ timeout + no-referrer; broken saved links self-drop so the procedural artwork always returns (the fallback rule); per-mug reset-to-artwork + global reset-all; photo badges on slots. Suite drives the REAL file input via DataTransfer. twin 15/15 ×2 + functional 17/17 + style 10/10 verbatim.
