...
## log
- 2026-05-17: v1.6 — **lockers, searchable chests, 3 items, and an MP downed/rescue mechanic** per chat ask: "add lockers and search-able chests to pixel-fog that provide items for teammate rescue." Built the full DBD-style loop: hide in lockers, search chests for items, items aid rescue, and getting caught in co-op no longer instantly ends the round — your teammate has 45 seconds to come back for you.
  - **Procgen extension**: `generateMap()` now also returns `lockers` (6) and `chests` (4). Both placed on widely-spaced floor cells from a shuffled best-region pass, excluded from gen positions / player spawn / each other (≥3 tile Manhattan separation between same-type, ≥2 between different types). New top-level `lockerSpawns[]` / `chestSpawns[]` parallel to `genSpawns[]`; populated in `applyGeneratedMap()`. Both arrays sent via `mpSendMapToPeer` and read by `mpApplyMap` so the joiner sees the same world layout.
  - **State extension**: `state.player` got `item` (1-slot inventory, null|'medkit'|'smoke'|'speed'), `hidden` (in locker), `lockerIdx`, `sprintUntil`, `searchTarget`, `downed`, `downedUntil`, `_rescueHold`. `state.lockers[]` (`occupiedByLocal/occupiedByRemote`), `state.chests[]` (`opened`, `search`, `item`), `state.smokeClouds[]` (transient LOS-blocking circles).
  - **LOCKERS**: Press **Q** within 30px to enter; **Q** again to exit. While hidden:
    - `drawPlayer` skipped (locker visually swallows you)
    - Movement/actions frozen (`updatePlayer` early-returns inside the `hidden` guard)
    - Killer can't see/hear you — `playerEvadable = p.hidden || p.downed` short-circuits the vision + noise checks. If the killer was chasing, entering a locker hard-breaks LOS: `chase → search` w/ `lastSeen` set to locker.
    - Catch check excludes hidden players: `if (!blinded && !p.hidden && !p.downed && distance < catchR) downPlayer()`.
    - Visual: 18×24 steel cabinet with 2 doors (vertical seam), 3 horizontal vent slats per door, top/bottom edge bands, tiny door handles, drop shadow. When occupied, a magenta pulsing rim (`rgba(192,96,255, 0.35-0.70)` at sin-pulse on `t/220`) telegraphs "someone's in there." Sync via `{t:'locker', idx, occupied}` — peer flips `occupiedByRemote` so you can't double-up.
    - Audio: `sndLockerClose` (180Hz square + 420Hz triangle), `sndLockerOpen` (520→280Hz pair).
  - **CHESTS**: Hold **E** within 30px for 3.0 seconds to open. Inventory must be EMPTY to start a search (visible chest pulse hint when in-range). On open: random item from `['medkit','smoke','speed']`, lid hinges back 99°, faint warm radial glow rises from the empty interior. Bandpass-noise creak (`sndChestCreak`, 1800Hz Q=2.4) plays periodically while searching. Open `sndChestOpen` = lid-pop + 220/660/990Hz arpeggio. Item pickup plays 4-note major arpeggio (`sndItemPickup` 523→1047Hz). MP sync: 3Hz progress broadcasts `{t:'chest', idx, opened:false, prog}`; immediate `{t:'chest', idx, opened:true, item}` on completion. Search progress arc drawn above the chest in the existing repair-arc style.
  - **ITEMS** (1-slot inventory, **Q** uses held item):
    - **🩹 Med-kit** — MP: instantly rescues a downed teammate within 60px (no 3s hold needed); broadcasts `{t:'rescue', instant:true}`. SP: becomes a one-time second-chance — `downPlayer()` checks for medkit, consumes it, shoves the killer 80px away, resets him to `search`, restores +1 flashlight charge. Toast `MED-KIT BURNED · you survived`.
    - **💨 Smoke bomb** — drops a 56px-radius cloud at your feet, lasts 4.5 seconds. Adds entry to `state.smokeClouds[]`. New `smokeBlocksLOS(ax,ay,bx,by)` does segment-vs-circle on every live cloud; called from `updateKiller`'s vision check, so killer literally cannot see through smoke. 7-puff radial-gradient render with offset drift over time; fades in over 250ms, holds, fades out over last 400ms. Sync via `{t:'smoke', x, y, until}`.
    - **⚡ Adrenaline** — 5 second sprint at 1.42× player speed (`p.sprintUntil = now + 5000`). Footstep cadence also speeds up (`200ms` instead of `280ms`) so the urgency is audible. Audio: 180Hz sine thump + 440/880Hz triangle whoosh.
  - **DOWNED & RESCUE (MP only)**:
    - Killer catching either survivor no longer triggers immediate lose. Instead, that player enters `downed` state at the catch spot with a 45 second `downedUntil` timer. While downed: frozen, can't act, broadcast position carries `dn:true` flag so peer immediately knows. Caught while in a locker → forcefully ejected (`l.occupiedByLocal = false`).
    - Teammate must walk within 30px and hold **E** for 3.0 seconds to rescue. Partial progress decays at 1.5× when E is released or out of range, so you can't tap-rescue. Green progress arc fills around the downed survivor while teammate holds.
    - Successful rescue: `{t:'rescue'}` broadcast; downed player gets cleared, rescue chime (523/659/784Hz major triad).
    - Timer expires (no rescue in 45s) OR both players caught simultaneously → both lose, with `{t:'lose'}` broadcast.
    - HUD shows pulsing red "**DOWNED · 12.4s · teammate must rescue (E)**" pill at bottom-centre for the downed player; remote teammate sees a bright pulsing X-marker + countdown floating above the fog so they can navigate to you even through the dark. SP catch: still triggers `lose()` immediately (no rescue partner).
    - New audio: `sndDownGroan` (descending 280→180→110Hz sawtooth), `sndRescueChime` (major triad up).
  - **E key contextual priority**: chest search (only if `inventory empty`) > teammate rescue > generator repair. This is rebuilt as a single decision tree at the top of `updatePlayer`, with explicit `doingChest`/`doingRescue`/`doingRepair` flags — much cleaner than the previous "wantRepair" single-purpose logic. Releasing E or moving out of range cancels chest mid-search (with partial decay) and resets rescue hold.
  - **HUD additions**: new "inventory · Q" card sits between flashlight and heartbeat — shows the item glyph + name + 'Q' key chip, or italic "— empty —" placeholder. Tooltip on hover gives the item's effect description. Refresh tied to the same ~4Hz throttle as the flashlight pip refresh. New "DOWNED" bottom-centre pill (z-25, pulsing red border) shows countdown when local player is down. Small ephemeral toast (z-25, amber-bordered pill) used for pickup confirmations + status messages — 2.2s default, dismissible.
  - **MP message types** added: `locker`, `chest`, `item`, `smoke`, `down`, `catch`, `rescue` (plus existing `pos` now carries `hd` for hidden + `dn` for downed flags). Total protocol now: `map/pos/killer/gen/noise/gate/win/lose/flash/blind/locker/chest/item/smoke/down/catch/rescue` (17 message types).
  - **Hint bar** updated: "WASD move · Shift sneak · E repair / search chest / rescue · Q locker / use item · F flashlight · P pause". Start panel key sheet also gets new E + Q rows.
  - File 84KB → ~110KB. Module script went 70KB → ~102KB.

- 2026-05-17: v1.5 — **suspenseful layered heartbeat audio loop + red screen pulse synced to proximity** per chat ask: "add a suspenseful heartbeat audio loop and a red screen pulse effect to pixel-fog that intensifies based on killer proximity."
  - **`killerCloseness` floor**: shared per-frame closeness signal, recomputed in the main loop on EVERY client (so the joiner — who doesn't run `updateKiller` — still gets the dread feedback). `closeness = max(0, 1 - dist/320)` + 0.18 bump when killer is in `chase`. Multiplied by 0.25 when the killer is blinded by a flashlight so the dread audibly + visually backs off as a reward for landing the stun. Joiner's `heartbeatRate` is back-derived locally from this same value so the beat timer stays in lockstep with the host without needing a sync packet.
  - **Suspenseful heartbeat audio layers** (was: lone dual-sine thud):
    1. **Dread drone** — two slightly-detuned 38 / 38.5 Hz sine voices, always running once audio resumes, gain target = `closeness^1.4 × 0.07`. Smooth `setTargetAtTime(t=0.25)` ramp so it swells/recedes naturally without zipper noise. Adds a constant cinematic floor of bass tension that scales with how close the killer is.
    2. **Per-beat kick transient** — fresh 50ms bandpass-filtered noise burst (`bandpass 90Hz Q=1.4`) fired at the start of each beat. Gain `0.08 + closeness × 0.18` so it punches harder when killer's near. Sounds like a stethoscope-mic'd chest thump under the sine LUB-DUB.
    3. **Original dual-sine lub-DUB** preserved: 80Hz @ 100ms + 56Hz @ 180ms (delayed 40ms). Volume still scales with bpm.
    4. **Tinnitus dyad whine** — two sine voices at 880Hz + 1320Hz (perfect fifth — sounds wrong / urgent at that register) through a 1100Hz Q=4 bandpass, gain GATED open only when `closeness > 0.62`. Target `min(0.03, (closeness-0.62) × 0.12)` — barely-audible but psychologically loud cue that catch range is about to be hit. Fast 0.15s attack into the gate, faster 0.08s release out.
  - **Red screen pulse** — full-viewport overlay `<div id="red-pulse">` with `z-index: 4` (above canvas, below HUD z=5/overlays z=20). Two-tone radial gradient: transparent centre at 32%, `rgba(180,16,28,0.65)` at 78%, dark `rgba(80,4,10,0.90)` at 100% — so the danger tint hugs the screen edges and keeps the play area readable. Stacked under a top/bottom linear gradient for a slight letterbox dread frame. `mix-blend-mode: screen` lifts the red against the dark game palette without nuking the canvas content. 70ms linear opacity transition keeps the in-between motion smooth.
  - **Two-channel opacity model**:
    - **Baseline tint** = `closeness^1.6 × 0.30` — almost invisible until killer's actually near, swells smoothly to 30% near catch range.
    - **Per-beat spike** = `pulseRedHeart(closeness)` called from inside `beat()` — sets `redSpike = 0.35 + closeness × 0.85`. Spike exp-decays at rate `~3.6/s` so it dies within ~600ms (just before the next beat). Composed each frame as `max(baseline, redSpike × (0.40 + closeness × 0.60))`. Hard-capped at 0.68 alpha — even at full chase, the screen never goes solid red.
    - DOM write throttle: only sets `style.opacity` when value changes by ≥0.02. Avoids per-frame layout work when the pulse is decaying smoothly.
  - **Accessibility**: detects `prefers-reduced-motion` at boot. When on, `pulseRedHeart` is a no-op (no rapid flashing) and the pulse instead shows a STEADY ambient tint scaled to 1.5× the baseline (still telegraphs danger via intensity, never flashes). CSS `transition` also lengthened to 400ms so the steady tint eases in/out instead of snapping.
  - **Visual + audio in lockstep**: both the per-beat audio kick AND the red flash are triggered from the same `beat()` callback, so the player feels a literal "my heart just thumped" combined sensation. Spike decay timing (~600ms) is calibrated to match a 100bpm beat interval so close-quarters chases feel like one continuous strobe-and-thud cycle.
  - File 78KB → ~84KB.
