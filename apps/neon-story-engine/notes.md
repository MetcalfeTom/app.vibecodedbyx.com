...
## log
- 2026-05-08: **Director was failing 100% of turns — TWO root causes** (chat ask: "check the logs for why every director response is still failing despite our strip and retry logic" + "search the neon-story-engine code for any hardcoded JSON fields that might be causing these persistent parsing failures" + "print out the raw model response for every agent call"). **Cause 1**: `response_format: { type: 'json_object' }` is broken on Pollinations. Verified via direct curl (`text.pollinations.ai/openai`): identical messages array with response_format → returns a CACHED canned greeting response ("Hey there! How can I help you right now?") with reasoning field showing the model thinks the user said "Please provide a response." (i.e. the messages array gets dropped server-side). Without response_format the same call returns clean JSON. Fix: dropped response_format from `pollPost` entirely (kept it on `openrouterCall` where it works). **Cause 2**: the hardened Director system prompt enumerated literal banned phrases ("Sure thing!", "Here's the JSON:", "Certainly,", "Of course!", "Let me…"). Pollinations content-hashes the messages and returns a CACHED greeting response containing those exact phrases when they appear in the prompt. Verified: same long prompt with the enumerations removed returns perfect JSON (`{"outcome":"success","changes":{...},"brief":"...","npc_speaks":null}`). Fix: kept the structural rules (first char `{`, last char `}`, no preamble, no fences, no trailing prose) but no longer name the phrases; the stripPreamble regex chain at the fetch boundary still catches them if a model slips one through, so the safety net stays in place. **Diagnostic logging**: every agent call now console.logs its outgoing payload key + the raw response (truncated to 600 chars) + finish_reason via `extractAssistantText` so chat can paste a per-turn trace from DevTools — `[story-engine][director] sending → ...` / `[story-engine][director] raw content · N chars · finish: stop · {"outcome":"success",...}` / `[story-engine][director] parsed → {...}` and analogous lines for narrator + npc-voice. **Field-name audit** (third chat ask): the schema in the system prompt, regexExtractDirector, normaliseDirector, and runMemory all reference the same keys (`outcome`, `brief`, `changes.{location, inventory_add, inventory_remove, cast_meet, flags_set, time_advance}`, `npc_speaks.{name, line_brief}`) — no typo or hardcoded-field drift; that was not the cause of failures.
