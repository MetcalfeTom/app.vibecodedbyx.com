...
## log
- 2026-08-13: v1.1 — **draggable resize** (chat): authentic Win95 corner size grip (three CSS diagonal ridges, nwse cursor) in the statusbar corner. Drag grows the window right/down as 1995 intended (window pins on grab); grid canvas height flexes 120–520px, width clamps 420–viewport; canvas backing store re-syncs on resize so blocks stay crisp; keyboard arrows resize the focused grip (quiet a11y). Verified 10/10: grip styled, drag +120/+90 grows both dimensions, canvas synced, blocks redraw at new size, min clamps hold under absurd shrink, keyboard resize, legend/progress/status layout intact, gradient titlebar + bevels preserved, engine runs after resize, titlebar drag unaffected.
