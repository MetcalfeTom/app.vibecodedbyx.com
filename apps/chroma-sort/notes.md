...
## log
- 2026-04-21: Added **Letters mode** (difficulty `letters` — 16 rounds, 4×4 grid, penalty 3). Each round picks 16 random letters from A–Z, sorts them alphabetically, then maps position in the alpha order → hue (0→340° in 16 steps, 78% sat, 56% light) so correct sort reveals a rainbow. Tiles render the letter as one huge character via `.is-letter` class (Anybody 900, clamp(30px,9vw,88px), glow + drop shadow). Sort uses `name.localeCompare` on the letter name directly. Newround branch returns early with its own setup path.
