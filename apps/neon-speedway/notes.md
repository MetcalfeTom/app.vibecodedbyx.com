...
## log
- 2026-04-25: **Fix: empty track on load + Twitch debug breadcrumbs** (public report — "no cars spawning"). Three fixes: (1) Auto-start demo mode on boot via `window.addEventListener('load', ...)` 600ms after first paint if `racers.size === 0`. Real Twitch chat preempts demo (`stopDemo(true)` on JOIN ack). On WSS close demo resumes so track is never blank. (2) `BASE_SPEED` constant → `baseSpeed()` function returning `max(60, perimeter/30)` so a base lap is ~28-32s on any viewport (was 117s on desktop). Boost cap raised to `max(220, baseSpeed()*4)`. Decay 0.78→0.85, lerp 4.5→5.5/1.6→1.8 for snappier feel. (3) Connect handshake adds `console.log('[neon-speedway] WSS open, JOIN #channel as justinfanXXX')` and per-PRIVMSG `chatStats.received++` so DevTools shows whether messages are landing. Fixed `displayName` regex comment — `\\s` already correctly matches Twitch's literal `\s` escape. Status text updated: "✅ live · #channel · waiting for chat" so users know the connection landed even if chat is silent.
