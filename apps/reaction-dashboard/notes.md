...
## log
- 2026-04-24: **Multiplayer broadcast.** Added Supabase Realtime channel `reaction-dashboard-v1` (broadcast + presence, **no DB writes**). Every button press fires locally AND emits `{type:'broadcast', event:'reaction', payload:{id, from:clientId}}` — all other connected viewers receive it and trigger the same emoji rain + sound + button press animation. Refactored `trigger(id)` into `fireReaction(id, remote)` (local-only, also used for remote replays) + `trigger(id)` wrapper that also calls `broadcastReaction(id)`. Remote-triggered presses use `.remote-pressed` class (lime-green pulse-ring keyframe) instead of the local `.pressed` magenta ring so you can see at a glance whether *you* pressed a button or someone else did. Self-filtering via 8-char random `clientId` passed in payload + channel config `broadcast:{self:false}`. 50ms debounce against key-repeat spam. **Presence**: tracks `{online_at}`, `sync` event updates a live-viewer count pill in the footer (lime dot `livePulse` keyframe when connected, yellow when connecting, pink offline/error). `<script>` → `<script type="module">` to support ESM `createClient` import from jsdelivr. No auth required — ephemeral broadcast channels allow anon clients to send/receive. Fallback on CHANNEL_ERROR/CLOSED/TIMED_OUT shows "—" + offline styling.
