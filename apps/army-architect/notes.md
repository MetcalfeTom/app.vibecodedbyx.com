...
## log
- 2026-04-24: Added **Versus mode** (local hot-seat). Mode toggle above palette: `vs AI` (original wave mode) / `vs Rival` (two-player). In vs mode: both commanders get 100g, Commander deploys on left half → clicks `› Pass to Rival` → fullscreen ink-black pass overlay ("Rival, take the field — shield your eyes") → Rival clicks "I am ready" → deploys on right half → clicks `⚔ Commence Battle` → same sim runs, winner screen says "Commander Wins" / "Rival Wins" + Rematch button. Per-side gold pools (`goldLeft` vs `goldLeftEnemy`), `state.side` tracks whose turn it is. Refactored placement helpers: `sideZoneOK(x, side)` + `spendGold/refundGold/goldFor(side)` so Auto-Muster, Disband All, right-click refund, overlap check, and ghost preview all respect the active side. Zone labels fade the inactive side to 0.28 alpha + highlight the active side to 0.6. Preview now colors red when overlapping or unaffordable. **Unit selection fixes**: palette click no longer rebuilds DOM (was causing lost hover/title) — switched to in-place `.selected` class toggle via `selectUnit(key)` + `dataset.key`. Added numeric keyboard shortcuts (1-6) for quick unit pick with faint index hint next to unit name. Placement overlap margin raised from +2 to +3, and now checks **all** units (not just same side) so you can't stack on enemy sprites. Added bounds check `y < r+2 || y > h-r-2` so you can't place half-off-field. Right-click removal is side-scoped + hit radius `u.r+6` for easier targeting.
