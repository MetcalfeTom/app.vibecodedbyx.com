...
- 2026-08-18 (4): v1.3 — TABS per chat (asked twice, same shape): 🧫 timeline / 🎮 neighbors / 🧹 level 2. Zero feature-logic changes — setTab() toggles a .mwhidden class (class-based !important hide because #foods is display:flex and would shrug off UA [hidden] — the lesson bank strikes again) over section groups {timeline: foods+card+rules+compost, game, level2}. Choice persisted 'mold-watch-tab', default timeline. Tab suite 6/6 incl. COMPUTED visibility checks; full regressions behind tabs: timeline 17/17, neighbors 16/16, level 2 25/25 (64 checks). Timeline visuals verified unchanged by screenshot.
- 2026-08-18 (5): v1.4 — THE FRIDGE CAMPAIGN (4th tab) per chat: manual snapshots, privacy-first opt-in, mystery leftovers, smell/fuzz/leak clues, cleanup choices.
  - 7 days, 6 containers (3 dated, 3 seeded MYSTERY tubs age 2–5 unknown), 2 "looks" per day (inspect or toss), 📸 manual snapshot is the ONLY thing that advances the day (probed).
  - **Clues only incriminate — never clear** (the house law extended): whiff (position-vague), sticky ring (names container), fuzz (names + "toss today"); fuzz>leak>smell ranking; quiet days say "quiet proves nothing — the four-day rule still applies". Suite audits every clue string across 6 seeded campaigns (60+ clues) for clearing words.
  - **Privacy-first**: opt-in checkbox OFF by default — probed that NOTHING hits localStorage while opted out even after playing; opt-in persists device-only; "forget everything" wipes + restarts + unchecks. Copy: "either way, nothing ever leaves your browser".
  - Mechanics: overnight aging + spread (fuzzy container +0.5/day to shelf neighbours); leaky toss opens the cleanup choice (wash costs the second look, +10; unwashed leak −8 at report); scoring enforces the REAL leftovers rule (unlabeled >4 days = toss, −12 if still resident) and charges waste for tossing fresh dated food (−10). Grades A–D; report closes "clues never said safe — nothing can."
  - Suites: camp-probe 17/17 (first run!), tabs 6/6 (updated for 4 rooms), timeline 17/17, neighbors 16/16, L2 25/25 — 81 checks.
