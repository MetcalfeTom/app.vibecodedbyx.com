...
## log
- 2026-04-22: Added **Vampire** class (plum `#7a2b6b`) + enlarged arena (`Math.min(W,H)*0.5`, up from `0.44`). **Player Vampire**: HP 100, speed 215, radius 20. Primary click-to-fire plum "blood bolt" (340u/s, 12 dmg, 0.25 radius, 0.42s CD, `lifesteal:0.5` on bullet) that heals player by `round(dealt*0.5)` on each enemy hit (floater shows `+N` in plum, tiny red particle poof). Secondary **Feast** (Space, 5s CD, 0.15s i-frames): expanding 140-radius drain ring (36 particle ring telegraph) damages all enemies in range for 38, then heals player by `round(totalDealt*0.8)` — empty arena → ring still visible, no heal. Render: rotating plum aura ring + plum body + dark inner core + ember eye aimed at cursor + 2 tiny white fangs. **Enemy Vampire**: contact rusher with `enemyContact:18`, `enemyHeal:10` self-heal per hit (0.55s CD + knockback + red particles + plum floater). Spawns at wave 3+ (15% at w3–4, 25% w5+). Cards grid expanded to 4-column (2-col ≤1020px, 1-col ≤560px). Added bolt + feast SFX.
