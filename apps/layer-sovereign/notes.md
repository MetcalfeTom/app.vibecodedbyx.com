...
- 2026-02-04: RLS audit + locked table handling. Tested all 38 tables with fresh anon JWT. Found 7 tables with read-own RLS (profiles, follows, mentions, karma, badges, comment_votes, global_backgrounds) — returns 0 even with valid auth for anonymous users with no rows. Created new `sloppy_identities` table with open read-all RLS as replacement identity source. Added `locked` flag to restricted tables, skip them in fetch, show lock icon + dash instead of fake 0. 39 tables total now (38 original + sloppy_identities).
...
- +7 locked tables (read-own RLS): sloppygram_profiles, sloppygram_follows, sloppygram_mentions, sloppygram_karma, sloppygram_badges, sloppygram_comment_votes, sloppygram_global_backgrounds. Cannot fix RLS with available MCP tools. Created sloppy_identities as open alternative.
...
- 39 tables (38 sloppygram_* + sloppy_identities), 7 locked (skipped in fetch), ~32 active count queries in batches of 8
