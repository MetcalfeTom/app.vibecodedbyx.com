...
## log
- 2026-06-28: **rewrote generator → one large central symmetrical mass (like real Rorschach cards)** (chat ask). Removed the scattered splatter (18–45 dots), the far satellite blobs, and the tendrils. Now: a single SOLID uniform ink colour (so overlaps don't seam) fills a chain of `nodes` (6–9) overlapping `smoothBlob`s down a near-fold spine — `cx = W/2 − (7..23) − sin(t·π)·W·0.17` so it hugs the centre line and bulges through the mid-body, with end lobes; 3–5 extra lobes branch off a spine node (overlapping it → stay connected); 0–2 tiny specks hug the mass for authenticity. Left-half clip + mirror = a symmetric central blot. Verified mathematically that consecutive spine nodes always overlap (`dist ≤ r_i + r_{i-1}`) → guaranteed single connected mass, not scattered.
