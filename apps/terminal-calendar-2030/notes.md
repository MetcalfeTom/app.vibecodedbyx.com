...
## log
- 2026-08-07: **vault-dice revival** (chat rolled 1-of-1496 → this app; shipped all 4 dormant todos). (1) `note <text>` command pins a personal entry (240-char cap) to the selected date — localStorage `tc2030-notes`, ✱ marker on the cell, aria-label "(has note)", note typed after the fiction log on read; `note -` erases. (2) Arrow-key navigation between day cells (focus-moving, Enter reads natively), grid-aware ±1/±7. (3) NOW button + `today` command jump to the real current month, auto-select + read today, permanent phosphor ring on today's cell. (4) ◉ button + `theme` command cycle green→amber→white phosphor (CSS var swap incl. parametrized CRT glow --bg2, persisted `tc2030-theme`). New terminal command line under the output (`help` lists commands, unknown commands sass back). Probe 14/14: select→note→marker→persist→erase, kb right/down, today jump+ring+select, theme cycle+persist+wraparound, no crashes. Scratch → atomic deploy.
