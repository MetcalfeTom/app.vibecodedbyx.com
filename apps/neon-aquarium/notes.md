...
## log
- 2026-09-26: **Phones: the tank finally fits.** The canvas is a flex item with no CSS width, so its minimum width came from its bitmap's 2:1 aspect ratio (the default 300×150) times the stretched height — on a 390×760 phone it was 1520 px wide and you only saw the left quarter; the +Fish sidebar and the in-flow 🏆 button sat off-screen (on most desktop windows too, whenever width < 2× height). Fix: `canvas{min-width:0;min-height:0}` plus a ResizeObserver that refits the bitmap 120 ms after the sidebar slides. `#lb-btn` hidden: its CSS targeted `a#lb-btn` but it's a <button>, and the `aquarium_leaderboard` table was never created.
- 2026-09-26: **first-visit how-to** (`#howto`, Fela's "help info" idea): a glassy card at the bottom — tap the water / tap a fish / +Fish — fades in at 0.9 s and out 1.8 s after the first tap or at 12.9 s; `aquarium_howto_seen` in localStorage so it shows once. One line on desktop (· separators), one tip per line under 560 px.
- 2026-09-26: **Jelly Bloom** mode (third piece, from Zen Aquarium): Modes → Jelly Bloom releases 4 giant pulsing jellies (`bloom[]`, `makeBloomJelly/updateBloom/drawBloomJelly`) ported from Zen's drawJelly — dome bell with concave underside, highlight arc, 4 inner ribbons, frill, 5–8 swaying tentacles + oral arms. Physics in Zen's px/second units with dt 0.016: pulse thrust, drift toward a wandering targetX, kept between 15% and 62% of the height, fade in over ~3 s. Toggling off makes them sink and fade out. Tentacle glow is a wide faint stroke + thin bright stroke under 'lighter' instead of shadowBlur (cheap on phones). Drawn behind the bio-school, decor and fish. Not saved in localStorage.
- 2026-09-26: **Schooling** (second piece from the other tanks, after Retro Aquarium's boids). Small fish (size <16, not jellies) steer toward same-species neighbours within 120 px: alignment 0.03, cohesion 0.0008 × mean offset, separation under 24 px 0.04. New **School of 6** button at the end of Add Fish: six small (7–10) fish of one random schooling species (Tetra/Guppy/Neon/Danio/Data) released together heading the same way (respects the 30-fish cap). Headless: two schools keep a mean radius of ~20 px after 40 s of simulated swimming.
