# hundred-and-one-followers

## log
- 2026-05-06: forked from `hundred-followers` and rebranded for the 101-follower encore. Counter target bumped 100 → 101; titles, OG tags, eyebrow, tagline, and OG URL all renamed. Fireworks engine got a substantial upgrade — see "updated fireworks" below.

### updated fireworks (chat ask)
- New **explosion kinds** chosen per-rocket via `pickExplosionKind()` (18% chrysanthemum / 12% heart / 10% crackle / 25% ring / 35% peony):
  - **chrysanthemum** — long-trail willow drip. Lower drag (0.94) + heavier gravity (220) + 1.6-2.6s lifetime so sparks streak down like falling embers. Sparks carry `hasTrail:true` so the renderer draws a fading polyline of the last 7 positions behind each.
  - **heart** — parametric heart curve `x = 16sin³t, y = -(13cosT − 5cos2t − 2cos3t − cos4t)` mapped to ~70 sparks evenly along the perimeter, scaled by `speed × 0.085` so the heart fits the explosion size. Sparks also carry the brocade trail.
  - **crackle** — peony scatter where each spark mid-life (40-75% through) splits into 4 mini-sparks, giving a "snap! snap! snap!" secondary cascade after the first burst.
  - **ring / peony** — kept as before for variety.
- **Multi-stage shells**: 25% chance per rocket — after the primary burst, `fireSubShells()` schedules 3 sub-shells that fly outward from the burst at random angles with their own 0.45-0.75s fuses; each detonates with its own randomly-picked explosion kind (so a single shell can chain into 4 distinct bursts). Sub-shells render as glowing tracer dots in flight.
- **Brocade trails**: chrysanthemum + heart sparks store the last 14 path points (7 segments) and the renderer draws a decaying polyline behind each at 40% alpha, in the spark's own color. Performance-cheap: only updates every ~18ms.
- **Counter target** bumped to 101 (2.52s ease-out) so the celebratory burst now also pulls a `fireSubShells` chain ~1s after the heart + chrysanthemum + crackle anchor explosions. The headline tagline reads "one more came in. it's a hundred and one now — chrysanthemums, hearts, multi-stage shells. this round's on the house."
- Original `hundred-followers` left untouched as a sibling app — old links + bookmarks still resolve. If chat wants to retire it, that's a separate ask.

### inherited from hundred-followers (still present)
- Counter animation: 0 → 101 ease-out-cubic with synth tick every 5 numbers.
- Bungee Shade title with 3D pink/violet shadows + Bungee Inline headline. Full-window canvas of fireworks + confetti behind a layered DOM banner.
