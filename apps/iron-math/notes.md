...
## log
- 2026-08-06 v2.3: scattered-floor pass (2 chat msgs) — 9 kg plates (2.5-25kg, sizes 18-34vmin, 13 distinct rotations, inline styles, font-size=size*0.16vmin drives .cast em sizing) + 3 lb-native (2x45LB/20.4KG, 10LB/4.5KG — lb-first stamps) + .plate.sc debris shadows (resting drop + offset spill + rim rust-grit line). Stamp-math invariant now tested BOTH directions (kg*2.20462 and lb/2.20462, tol .06). Visual passes on this app: 8 — if chat asks for more plates, generate them in JS instead of more inline HTML.
- 2026-08-06 v2.0: full backdrop redesign per chat — .plates-bg (fixed, z0; .book z1): .plate.p1 90vmin centered + .p2 52vmin corner; cast stamps (.cast, Staatliches, 13% emboss via dual text-shadow) in BOTH units — **stamp values must satisfy kg×2.20462=lb** (25/55.1, 20/44.1 — verified in test); hub ::after, rim ::before, 5 rust radials + machining rings. Spiral: **inline SVG wire-loop tile** (urllib-quoted data URI; tilted ellipse stroke w/ steel linearGradient + dash glint threading a punched hole), 2.5×2.125rem repeat-y. Floor: +2 rust radials + 37° rust-streak repeating-linear. If editing the SVG, regenerate the quoted URI — don't hand-edit percent encoding.
