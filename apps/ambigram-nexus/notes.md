...
## log
- 2026-05-08: vertical-centre fix + SVG/PNG export. **Centre fix**: rotated B-chars in the overlay panel now share a single rotation axis at B's whole-word line midline (`Bbnd.mid`) and translate by a CONSTANT `dy = Abnd.mid - Bbnd.mid` — guarantees the overall vertical centre of word B (rendered) coincides exactly with the overall vertical centre of word A regardless of which letters carry ascenders/descenders (chat: "ensure the vertical centers match exactly"). X is still aligned per-character by glyph-bbox centre so each rotated B-glyph still lands in its corresponding A-slot horizontally. Replaces the prior per-char-bbox-centre approach which gave each char its own vertical axis. **Export**: two new gold buttons in the overlay panel head — `↓ svg` writes a standalone XML w/ inlined background rect + glyph fills + screen-blend on the cyan layer (so the file renders identically to the on-page panel without our CSS), `↓ png` rasterizes the same standalone SVG at 2400px wide via Image→Canvas→toBlob (height scales to overlay aspect ratio so nothing clips). Filename slug = `ambigram-<wordA>-<wordB>.{svg|png}` or just `ambigram-<wordA>` for self-ambigrams; non-alphanumeric chars stripped.
