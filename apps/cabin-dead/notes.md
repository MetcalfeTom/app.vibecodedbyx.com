...
- 2026-04-08: Added Mystery Box at tile (6.5, 6.5) — floating crate billboard sprite with glowing "?" and orange halo, drawn per-column with z-buffer test. Press E within 1.5 tiles to roll ($500). Spins through weapon names with decelerating ticker over 2.8s then grants a random one. 8s cooldown before box can be rolled again. 7 weapons: SHOTGUN (default), PISTOL, SMG, RIFLE, SNIPER (high dmg/slow), MINIGUN (100 clip), RAY GUN (purple muzzle flash). Each has distinct dmgMin/dmgMax, cooldown, clip, reserve, reloadTime. drawWeapon() switches on state.weaponKey to render unique silhouette per weapon. Weapon name shown in HUD, box prompt pulses near player, roll banner shakes mid-screen during spin.
- 2026-04-08: Bumped raycaster internal resolution 400×240 → 720×432, enabled canvas imageSmoothing ('high') so upscale now uses bilinear instead of nearest-neighbor. CSS `image-rendering: auto`. Raycasting loop now steps by 2 columns and fills a 2px stripe + mirrors zbuf, keeping ray count the same while doubling vertical/horizontal pixel budget for sprites and HUD.
